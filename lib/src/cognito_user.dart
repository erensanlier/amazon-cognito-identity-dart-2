...
class CognitoUser {
  ...
  Future<CognitoUserSession?> authenticateUser(
    ...
    if (authenticationFlowType == 'USER_PASSWORD_AUTH') {
      ...
      return await _authenticateUserDefaultAuth(authDetails, uniqueKey);
    }
  ...
  Future<CognitoUserSession?> _authenticateUserDefaultAuth(
    ...
    dynamic value = await loginWorker?.post('getLargeAValue', uniqueKey);
    ...
    dynamic hkdfValue =
    await loginWorker?.post('getPasswordAuthenticationKey', uniqueKey, {
      'username': username,
      'password': authDetails.getPassword(),
